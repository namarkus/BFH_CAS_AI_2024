...
def load_and_prepare_time_series(station_id: str, work_path: str, station_data: pd.DataFrame, oldest_required_date: pd.Timestamp):
    """
    ...
    station_recordings['date'] = pd.to_datetime(station_recordings['date'], format='%Y%m%d')
    station_recordings = station_recordings[station_recordings['date'] >= oldest_required_date]
    if station_recordings.shape[0] == 0:
        print(f'Keine Daten für Messstation {station_id} im gewünschten Zeitraum seit {oldest_required_date:%d.%m.%Y} vorhanden')
        return pd.DataFrame(), pd.DataFrame()
    # Datenbereinigungen:
...
station_filter = load_station_data(WORK_PATH, country_filter)
station_data = pd.DataFrame()
oldest_required_date = pd.Timestamp.now() - pd.DateOffset(years=YEARS_TO_KEEP)
print(f'Alle Daten vor dem {oldest_required_date:%d.%m.%Y} werden herausgefiltert')
for iso_country_code in country_filter['iso_country_code']:
    country_flat_data = pd.DataFrame()
    country_grouped_data = pd.DataFrame()
    stations_in_country = station_filter[station_filter['iso_country_code'] == iso_country_code]
    ...
    country_stations = station_filter[station_filter['iso_country_code'] == iso_country_code]
    for station in country_stations.itertuples():
        station_id = station.station_id
        if station.latitude > frankfurt_latitude:
        ...
        station_flat_series, station_grouped_series = load_and_prepare_time_series(station_id, WORK_PATH, station_filter, oldest_required_date)
        print(f'Verarbeitung für Messstation {station_id} mit {station_flat_series.shape[0]} Datensätzen abgeschlossen')
        if station_flat_series.shape[0] > 0:             
            station_data = pd.concat([station_data, pd.DataFrame([station._asdict()])], ignore_index=True)
...
print(f'Alle Daten verarbeitet und gespeichert. Es verbleiben Daten von {station_data.shape[0]} Messstationen seit {oldest_required_date:%d.%m.%Y}.')
